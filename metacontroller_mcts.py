...
# Global declaration for Sliding window UCB (non-uniform):
LAST_CHOICES = []
LAST_REWARDS = []
TAU = 1000
...
class SearchNode(object):
  ...
  """
    ...
    def slide_uct_value(self, parent_explore_count, uct_c):
        """Returns the Sliding window UCT value of child."""

        global PARENT_EXPLORE_COUNT
        PARENT_EXPLORE_COUNT =parent_explore_count

        last_action = np.count_nonzero(LAST_CHOICES == self.action)
        ...
        return self.total_reward / last_action + uct_c * math.sqrt(
            math.log(min(parent_explore_count, TAU) / last_action))
    ...
    """
        ...
        for _ in range(self.max_simulations):
            ...
            for node in reversed(visit_path):
                ...
                # Sliding the window
                current = PARENT_EXPLORE_COUNT % TAU

                reward = returns[root_player if node.player ==
                                                pyspiel.PlayerId.CHANCE else node.player]

                global LAST_CHOICES, PARENT_EXPLORE_COUNT
                LAST_CHOICES[current] = node

                global LAST_REWARDS
                LAST_REWARDS[current] = reward

                PARENT_EXPLORE_COUNT += 1
