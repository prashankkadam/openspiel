...
def prepare_elo(data, cols):
  ...
  # df["P1_wins"] = df["total_trajectories"] * df["Player1"]
  # df["P2_wins"] = df["total_trajectories"] * df["Player2"]
  return df


def plot_elo(ax, x, data, data_col, normalized=True):
  """Plot stats produced by open_spiel::HistogramNamed::ToJson."""
  names = subselect(data, [0] + data_col + ["names"])
  df = prepare_elo(data, {name: data_col + ["counts", i]
                      for i, name in enumerate(names)})
  # if normalized:
  #   total = sum(df[n] for n in names)
  #   for n in names:
  ...
  for index, row in df.iterrows():
    ...
    for i in range(int(outer)):
      if i < int(inner):
        Ra, Rb = EloRating(Ra, Rb, K, win_check1)
      Ra, Rb = EloRating(Ra, Rb, K, win_check2)
    df.at[index, 'P1_elo'] = Ra
    df.at[index, 'P2_elo'] = Rb

  df.plot.line(ax=ax, x='serial', y=['P1_elo', 'P2_elo'], color={'P1_elo':"red", 'P2_elo':"blue"})
  print(df)
...
def plot_data(config, data):
  ...
  ax = subplot(num_rows, num_cols, 3, title="Elo Ratings")
  plot_elo(ax, x, data, ["outcomes"])

  ax = subplot(num_rows, num_cols, 4, title="Outcomes", ylim=(0, 1))
