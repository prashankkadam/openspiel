"""
Created on Tue 06/04/2020 00:07:20
...
"""
Metacontroller algorithm from the Agent-57 paper fitted over openspiel
...
class SearchNode(object):
  ...
  """
    ...
    # last_choices, last_rewards, parent_explore_slide
    ...
    def slide_uct_value(self, parent_explore_count, uct_c, tau_c,
                        last_choices, last_rewards, parent):
        """Returns the Sliding window UCT value of child."""

        # self.parent_explore_slide = parent_explore_count
        last_action = np.count_nonzero(last_choices == parent)
        final_reward = np.sum(last_rewards[last_choices == parent])
        ...
        return final_reward / last_action + uct_c * math.sqrt(
            math.log(min(parent_explore_count, tau_c) / last_action))
...
class MCTSBot(pyspiel.Bot):
    ...
    def __init__(self,
                 ...
                 child_selection_fn=SearchNode.slide_uct_value,
    ...
    Args:
      ...
      tau_c: Window size for the sliding window UCB.
      greedy_e: The exploration constant for greedy epsilon.
    ...
    """
        ...
        self.last_choices = np.zeros(tau_c, dtype="O")
        self.last_rewards = np.full(tau_c, -1, dtype="f")
        self.parent_explore_slide = 0
    ...
    """
        ...
        while not working_state.is_terminal() and current_node.explore_count > 0:
            ...
            if working_state.is_chance_node():
                ...
                # greedy epsilon:
                if random.random() >= self.eps[0]:
                    # Otherwise choose node with largest UCT value
                    chosen_child = max(
                        current_node.children,
                        key=lambda c: self._child_selection_fn(  # pylint: disable=g-long-lambda
                            c, current_node.explore_count, self.uct_c, self.tau_c[0],
                            self.last_choices, self.last_rewards, current_node))
                else:
                    chosen_child = random.choice(current_node.children)
    ...
    """
        ...
        for _ in range(self.max_simulations):
            ...
            for node in reversed(visit_path):
                ...
                # Sliding the window
                current = self.parent_explore_slide % self.tau_c[0]

                reward = returns[root_player if node.player ==
                                                pyspiel.PlayerId.CHANCE else node.player]

                self.last_choices[current] = node
                self.last_rewards[current] = node.total_reward - reward
                self.parent_explore_slide += 1
            ...
            if root.outcome is not None:
                # Resetting global variable after the loop ends
                # global PARENT_EXPLORE_COUNT
                # PARENT_EXPLORE_COUNT = 0
                break
            # Resetting global variable after the loop ends
            # global PARENT_EXPLORE_COUNT
            # PARENT_EXPLORE_COUNT = 0
